...
5. 可选：点 **选择文件** 挂一个 3D 形象文件（GLB/GLTF，≤20MB；按 Apple ARKit 52 表情标准或 Unity Humanoid 全身骨骼标准的可勾选对应选项）
...
- 如果你的模型用的是 **Apple ARKit 52** 表情标准（52 个 blendshape），勾上「支持 Apple ARKit 52 表情」，将来房间里的**表情驱动**就能直接对上。
- 如果用的是 **Unity Humanoid**（Mecanim 人形骨骼）标准，勾上「支持 Unity Humanoid 全身骨骼」，将来可以做**全身骨骼动画**（走路、挥手等）。两个标准可以同时勾选（面部表情 + 全身骨骼）。
