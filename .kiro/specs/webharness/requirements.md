...
1. 登录页与注册分离：登录页只有用户名/密码/登录按钮；「创建账号」打开独立注册弹窗（用户名、密码、确认密码、头像选择、3D 形象文件 + 标准标记：ARKit 52 表情 / Unity Humanoid 全身骨骼），注册成功自动登录。登录状态保存在浏览器本地，过期自动回到登录页。
...
9. 私聊消息渲染：正文 SHALL 去掉开头的 `@@用户名` 前缀（存储与 API 响应保持原样）；气泡头部 SHALL 按「发言人（头像+名称）→ 其他接收者（头像+名称）」排列；标记 SHALL 显示总人数（如「2人私聊」「3人私聊」），接收者列表由服务端消息字段 `whisperTo` 下发。私聊气泡 SHALL 保持虚线样式。
